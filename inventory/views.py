...
from .models import (
    Center, StockSheet, MasterItem, ItemAlias, StockEntry, CenterMasterItem, CenterItemAlias,
    PrintTemplate, PrintTemplateItem, CenterClassification, TargetReportConfig, TargetReportColumn,
    Notification, SupervisorDevice, SupervisorReminderSettings,
)
from .utils import (
    analyze_with_gemini,
...
def manage_centers(request):
    ...
    centers = Center.objects.select_related('classification', 'upload_token').prefetch_related(
        Prefetch(
            'supervisor_devices',
            queryset=SupervisorDevice.objects.filter(is_active=True).order_by('-paired_at'),
        )
    ).all().order_by('name')
    ...
    reminder_settings = SupervisorReminderSettings.get()
    ...
    return render(request, 'inventory/manage_centers.html', {
        ...
        'supervisor_app_url': request.build_absolute_uri('/inventory/supervisor/'),
    })
...
def admin_generate_upload_token(request, center_id):
    ...
    return JsonResponse({
        ...
        'url': upload_url,
        'pwa_url': request.build_absolute_uri(reverse('supervisor_app')),
